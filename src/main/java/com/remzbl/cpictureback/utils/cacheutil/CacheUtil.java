...
public class CacheUtil {
    ...
    public static final String MainPUBLICKEY = "cpicture:PublicMainPage"; // 公共图库主页缓存
    public static final String PUBLICKEYPRE = "cpicture:PublicCache:"; // 公共图库条件缓存键前缀

    public static final String MainPRIVATEKEY = "cpicture:PrivateMainPage"; // 私有图库主页缓存
    public static final String PRIVATEKEYPRE = "cpicture:PrivatePage:"; // 私有图库条件缓存键前缀
...
//    }
    ...
    /**
     * 更新缓存（ (上传)编辑图片后同步更新缓存 , 针对单个图片的缓存更新 ）
    ...
    public void updateCacheAfterEdit(PictureEditRequest pictureEditRequest, Long spaceId , int source) {
        ...
        // 修改图片所在的页面名称
        String currentPage = pictureEditRequest.getCurrentPage();
        ...
        //私有图库缓存
        // 主缓存
        String sCacheKey1 = null;
        // 分类 + 标签缓存
        String sCacheKey2 = null;
        //公共图库缓存
        // 主缓存
        String gCacheKey1 = null;
        // 分类 + 标签缓存
        String gCacheKey2 = null;
            // 上传默认在第一页构造key
        if(source == 1){

            //私有图库缓存
            // 主缓存
            sCacheKey1 = MainPRIVATEKEY + spaceId + ":" + 1;

            // 分类 + 标签缓存
            sCacheKey2 = PRIVATEKEYPRE + generateCacheKey(category, tags)  +spaceId + ":" + 1 ;

            //公共图库缓存
            // 主缓存
            gCacheKey1 = MainPRIVATEKEY + ":" + 1;

            // 分类 + 标签缓存
            gCacheKey2 = PUBLICKEYPRE + generateCacheKey(category, tags) + ":" + 1 ;

            // 原图编辑需要区分哪个页面
        }else{
            //私有图库缓存
            // 主缓存
            sCacheKey1 = MainPRIVATEKEY + spaceId + ":" + current;

            // 分类 + 标签缓存
            sCacheKey2 = PRIVATEKEYPRE + generateCacheKey(category, tags)  +spaceId + ":" + current;

            //公共图库缓存
            // 主缓存
            gCacheKey1 = MainPRIVATEKEY + ":" + current;

            // 分类 + 标签缓存
            gCacheKey2 = PUBLICKEYPRE + generateCacheKey(category, tags) + ":" + current;
    ...
    public void updateCacheAfterBatchEdit(PictureEditByBatchRequest pictureEditByBatchRequest ,Long spaceId) {
        ...
        List<String> oldTags = pictureEditByBatchRequest.getOldTags();
        String oldCategory = pictureEditByBatchRequest.getOldCategory();
        int current = pictureEditByBatchRequest.getCurrent();
        // 需要删除的缓存键
        String oldCacheKey = PRIVATEKEYPRE + generateCacheKey(oldCategory, oldTags) + spaceId + ":" + current;
        log.info("oldCacheKey:{}",oldCacheKey);
        ...
        //私有图库缓存
        // 主缓存
        String sCacheKey1 = MainPRIVATEKEY + spaceId + 1;
        ...
        // 删除旧的缓存
        // redis删除所有缓存
        Set<String> keys = stringRedisTemplate.keys("cpicture:Private*");
        if (keys != null && !keys.isEmpty()) {
            stringRedisTemplate.delete(keys);
            System.out.println("[私有缓存] 实时删除键: " + keys);
        }
        localCache.invalidateAll();

        if (sCachedValue1 != null) {
            //updateAfterBatchEdit(sCacheKey1,sCachedValue1,category, tags, valueOps);
            stringRedisTemplate.delete(sCacheKey1);
            localCache.invalidate(sCacheKey2);
     ...
     * @param spaceId
     */
//    public void updateCacheAfterDelete(PictureEditRequest pictureEditRequest, Long spaceId , int source) {
//
//        Long id = pictureEditRequest.getId();
//        String name = pictureEditRequest.getName();
//        String introduction = pictureEditRequest.getIntroduction();
...
//    }
     ...
     * @param spaceId   空间ID（用于匹配缓存Key）
     */
    public void updateAfterDelete(Long pictureId, Long spaceId) {
