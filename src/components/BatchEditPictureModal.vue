...
import { onMounted, reactive, ref, defineProps, withDefaults } from 'vue'
import {
...
// 打开弹窗
const openModal = (request: API.PictureEditByBatchRequest) => {
  visible.value = true
  formData.oldCategory = request.oldCategory;
  formData.oldTags = request.oldTags;
  formData.current = request.current;
  formData.currentPage = request.currentPage;
...
const handleSubmit = async (values: any) => {
  ...
  const res = await editPictureByBatchUsingPost({
    ...
    oldCategory: formData.oldCategory,
    oldTags: formData.oldTags,
