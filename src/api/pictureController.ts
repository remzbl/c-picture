...
/** listPictureVOByPageWithCache POST /api/picture/list/page/vo/cache */
export async function listPictureVoByPageWithHomeCacheUsingPost(
  body: API.PictureQueryRequest,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePagePictureVO_>('/api/picture/list/page/vo/homecache', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
 ...
 * 管理员触发开放已审核待开放的图片
 */
export async function openPictureUsingPost(
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseBoolean>('/api/picture/open', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      // 如果需要携带 token 通常会在拦截器统一处理，此处可不写
    },
    ...(options || {}),
  });
}
